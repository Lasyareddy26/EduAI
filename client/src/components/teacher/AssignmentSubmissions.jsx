...
function AssignmentSubmissions() {
  ...
  const submitGrading = async () => {
    try {
      const answersToGrade = Object.entries(gradingData).map(([questionId, data]) => ({
        questionId,
        marksAwarded: Number(data.marksAwarded) || 0,
        teacherComment: data.teacherComment || ''
      }))

      await axios.put(`http://localhost:3000/teacher-api/grade-submission/${gradingSubmission._id}`, {
  ...
  const statusBadge = (status) => {
    const styles = {
      'auto-complete': { bg: '#dcfce7', color: '#166534', text: '✅ Auto-Graded' },
      'pending-review': { bg: '#fef3c7', color: '#92400e', text: '⏳ Needs Review' },
    ...
    <div className="as-container">
            ...
            <tbody>
              {submissions.map((sub) => (
                <tr key={sub._id}>
                  ...
                  <td className="text-center">{statusBadge(sub.evaluationStatus)}</td>
                  <td className="as-score text-right">
                    <span className="as-score-badge">
                      {sub.finalScore !== undefined ? sub.finalScore : "N/A"}
                    </span>
                  </td>
                  <td className="text-center">
                    ...
                    >
                      {sub.evaluationStatus === 'pending-review' ? '✏️ Grade' : '👁️ View'}
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {gradingSubmission && (
        <div style={{
          ...
          <div style={{
            background: '#fff', borderRadius: '16px', padding: '30px', maxWidth: '700px',
            width: '100%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)'
          }}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
              <h3 style={{margin:0}}>
                📝 {gradingSubmission.studentId?.firstName} {gradingSubmission.studentId?.lastName}'s Submission
              </h3>
              <button onClick={closeGrading} style={{
                background:'none', border:'none', fontSize:'1.5rem', cursor:'pointer'
              }}>✕</button>
            </div>

            <div style={{display:'flex', gap:'15px', marginBottom:'20px', flexWrap:'wrap'}}>
              <span style={{background:'#f3f4f6', padding:'6px 14px', borderRadius:'8px', fontSize:'0.85rem'}}>
                Auto Score: <strong>{gradingSubmission.autoScore}</strong>
              </span>
              <span style={{background:'#f3f4f6', padding:'6px 14px', borderRadius:'8px', fontSize:'0.85rem'}}>
                Manual Score: <strong>{gradingSubmission.manualScore}</strong>
              </span>
              <span style={{background:'#ede9fe', padding:'6px 14px', borderRadius:'8px', fontSize:'0.85rem'}}>
                Final Score: <strong>{gradingSubmission.finalScore}</strong>
              </span>
            ...
            {gradingSubmission.answers.map((ans, i) => {
              ...
              return (
                <div key={i} style={{
                  border: '1px solid #e5e7eb', borderRadius: '12px', padding: '15px',
                  marginBottom: '12px', background: isManual ? '#fffbeb' : '#f9fafb'
                }}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:'8px'}}>
                    ...
                    <span style={{
                      background: isManual ? '#fbbf24' : '#a3e635', color: '#000',
                      padding: '2px 8px', borderRadius: '8px', fontSize: '0.7rem', fontWeight: 600
                    }}>
                      {qType.toUpperCase()} • {maxMarks} mark{maxMarks > 1 ? 's' : ''}
                    </span>
                  </div>

                  <p style={{margin:'5px 0', color:'#374151'}}>
                    <strong>Student's Answer:</strong> {ans.answer || <em style={{color:'#999'}}>No answer</em>}
                  </p>

                  {question?.correctAnswer && (
                    <p style={{margin:'5px 0', color:'#059669', fontSize:'0.9rem'}}>
                  ...
                  {!isManual && (
                    <p style={{margin:'5px 0', fontWeight: 600, color: ans.isCorrect ? '#16a34a' : '#dc2626'}}>
                      {ans.isCorrect ? `✅ Correct (+${ans.marksAwarded})` : `❌ Incorrect (0/${maxMarks})`}
                    </p>
                  )}
                  ...
                  {isManual && (
                    <div style={{marginTop:'10px', padding:'10px', background:'#fff', borderRadius:'8px', border:'1px dashed #d97706'}}>
                      ...
                      <textarea
                        ...
                        rows={2}
                        value={gradingData[ans.questionId]?.teacherComment || ''}
                        onChange={(e) => handleGradeChange(ans.questionId, 'teacherComment', e.target.value)}
            ...
            <div style={{marginTop:'15px'}}>
              <label style={{fontWeight:600}}>Overall Feedback (optional):</label>
              <textarea
                rows={3}
                placeholder="General comments for this student..."
                value={teacherFeedback}
                onChange={(e) => setTeacherFeedback(e.target.value)}
                style={{width:'100%', padding:'10px', borderRadius:'8px', border:'1px solid #ccc', marginTop:'5px', resize:'vertical'}}
              />
            </div>

            <div style={{display:'flex', gap:'12px', justifyContent:'flex-end', marginTop:'20px'}}>
              <button onClick={closeGrading} style={{
                padding:'10px 25px', borderRadius:'8px', border:'1px solid #ccc',
                background:'#fff', cursor:'pointer', fontWeight:600
              }}>Cancel</button>

              {(gradingSubmission.evaluationStatus === 'pending-review' || gradingSubmission.evaluationStatus === 'reviewed') && (
                <button onClick={submitGrading} style={{
                  padding:'10px 25px', borderRadius:'8px', border:'none',
                  background:'#7b2cbf', color:'#fff', cursor:'pointer', fontWeight:600
                }}>
                  💾 Save Grades
                </button>
              )}
