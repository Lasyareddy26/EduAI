...
function StudentDashboard() {
    ...
    try {
      ...
      submissions.forEach((sub) => {
        ...
        if (
          ...
          sub.evaluationStatus === "auto-complete"
        )
          gradedCount++;
      ...
      try {
        const badgeRes = await axios.get(
          `http://localhost:3000/student-api/badges/${userId}`,
        );
  ...
  const daysUntil = (iso) => {
    ...
    const diff = Math.ceil(
      (new Date(iso) - new Date()) / (1000 * 60 * 60 * 24),
    );
    ...
    <div className="sd-container">
      ...
      <div className="sd-charts-row">
            ...
            <div className="sd-badges-grid">
              {badges.map((badge, idx) => (
                ...
                >
                  ...
                  <span className={`sd-badge-tier-tag ${badge.tier}`}>
                    {badge.tier}
                  </span>
                </div>
              ))}
          ...
          {selectedBadge && (
            <div className={`sd-badge-detail ${selectedBadge.tier}`}>
              <div className="sd-badge-detail-left">
                <span className="sd-badge-detail-icon">
                  {selectedBadge.icon}
                </span>
              </div>
              ...
              <button
                className="sd-badge-detail-close"
        ...
        <div className="sd-chart-card">
          ...
          <p className="sd-chart-subtitle">
            Score distribution across all your attempts
          </p>
            ...
            <div className="sd-upcoming-list">
              {upcomingAssignments.map((a) => {
                ...
                return (
                  <div
                    key={a._id}
                    className={`sd-upcoming-row ${urgency}`}
                  ...
                  >
                    <div className="sd-upcoming-left">
                      ...
                      <div>
                        ...
                        <p className="sd-upcoming-meta">
                          {a.subject} • {a.questions?.length || 0} Q •{" "}
                          {a.totalMarks} marks
                        </p>
                      </div>
                    </div>
                    <div className="sd-upcoming-right">
                      <span className={`sd-deadline-badge ${urgency}`}>
          ...
          {recentSubmissions.length === 0 ? (
            ...
            <div className="sd-results-list">
              {recentSubmissions.map((sub) => {
                ...
                const pct =
                  totalMarks > 0
                    ? Math.round((sub.finalScore / totalMarks) * 100)
                    : 0;
                const grade =
                  pct >= 80
                    ? "excellent"
                ...
                return (
                  <div key={sub._id} className={`sd-result-row ${grade}`}>
                    <div className="sd-result-left">
                      ...
                      <div>
                        <p className="sd-result-title">
                          {sub.assignmentId?.title || "Assignment"}
                        </p>
                        <p className="sd-result-meta">
                      ...
                      {sub.evaluationStatus === "pending-review" ? (
                        ...
                        <>
                          <span className="sd-result-score">
                            {sub.finalScore}/{totalMarks}
                          </span>
                          <span className={`sd-result-pct ${grade}`}>
                            {pct}%
                          </span>
                        </>
                      )}
