...
function StudentSubmissions() {
  ...
  const openFeedback = (submissionId) => {
    ...
    axios.get(`http://localhost:3000/student-api/submission-detail/${submissionId}`)
      .then(res => {
    ...
    <div className="sub-container">
      ...
      {modalLoading && ReactDOM.createPortal(
        <div className="sub-modal-overlay">
          <div className="sub-modal-loading">Loading submission details...</div>
        </div>,
        document.body
      )}
        ...
        <div className="sub-modal-overlay" onClick={closeFeedback}>
          <div className="sub-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sub-modal-header">
              ...
              <button className="sub-modal-close" onClick={closeFeedback}>✕</button>
            </div>

            {/* Score Summary */}
            <div className="sub-modal-scores">
              <span className="sub-modal-score-chip">
                Auto Score: <strong>{selectedSubmission.autoScore}</strong>
              </span>
              <span className="sub-modal-score-chip">
                Manual Score: <strong>{selectedSubmission.manualScore}</strong>
              </span>
              <span className="sub-modal-score-chip highlight">
                Final Score: <strong>{selectedSubmission.finalScore}</strong>
              </span>
              <span className={`status-badge ${
                selectedSubmission.evaluationStatus === 'pending-review' ? 'warning' :
                selectedSubmission.evaluationStatus === 'reviewed' ? 'success' : 'success'
              }`}>
                {selectedSubmission.evaluationStatus === 'pending-review' ? '⏳ Pending Review' :
                 selectedSubmission.evaluationStatus === 'reviewed' ? '✅ Reviewed' : '✅ Auto-Graded'}
              </span>
            </div>
            ...
            <div className="sub-modal-questions">
              {selectedSubmission.answers.map((ans, i) => {
                const question = findQuestion(selectedSubmission, ans.questionId)
                const qType = question?.type || 'unknown'
                const isManual = qType === 'short' || qType === 'essay'
                const maxMarks = question?.marks || 1

                return (
                  <div key={i} className={`sub-modal-question-card ${isManual ? 'manual' : 'auto'}`}>
                    <div className="sub-modal-question-header">
                      <strong>Q{i + 1}: {question?.questionText || 'Question'}</strong>
                      <span className={`sub-modal-type-badge ${isManual ? 'manual-badge' : 'auto-badge'}`}>
                        {qType.toUpperCase()} • {maxMarks} mark{maxMarks > 1 ? 's' : ''}
                      </span>
                    </div>

                    <p className="sub-modal-answer">
                      <strong>Your Answer:</strong> {ans.answer || <em className="no-answer">No answer provided</em>}
                    </p>

                    {/* For auto-graded questions: show correct/incorrect */}
                    {!isManual && (
                      <p className={`sub-modal-result ${ans.isCorrect ? 'correct' : 'incorrect'}`}>
                        {ans.isCorrect
                    ...
                    {!isManual && !ans.isCorrect && question?.correctAnswer && (
                    ...
                    {isManual && (
                      <div className="sub-modal-grading-result">
                        <span className="sub-modal-marks">
                          Marks: <strong>{ans.marksAwarded}</strong> / {maxMarks}
                        </span>
                      </div>
                    )}
                    ...
                    {ans.teacherComment && (
                      <div className="sub-modal-teacher-comment">
                        <span className="comment-label">💬 Teacher's Feedback:</span>
            ...
            <div className="sub-modal-footer">
              <button className="sub-modal-close-btn" onClick={closeFeedback}>Close</button>
            </div>
          </div>
        </div>,
        document.body
